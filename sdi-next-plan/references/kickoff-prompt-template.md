...
Paste this at the start of the user's first session for the new work item, replacing the placeholders.

```
Implementing [Phase N / <slug>] of this project per docs/IMPLEMENTATION_PLAN_[PHASE_N | <slug>].md.
...
Substitute `[Phase N / <slug>]` with whichever naming applies — `PHASE_N` for discrete phases (greenfield/migrations) or `<slug>` for free-form work (features/maintenance in ongoing projects). Drop the line that doesn't match the user's tool.

## When generating the kickoff for a specific user

The output to the user includes the kickoff prompt with the right line picked, not both. Decide based on what the user said about their tool:
